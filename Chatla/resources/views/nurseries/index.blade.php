...
<body class="bg-background-light font-display text-slate-900 min-h-screen flex flex-col">
    ...
    {{-- ── Header (Official Chatla Style) ────────────────────────────────── --}}
    <header class="sticky top-0 z-50 bg-background-light/80 backdrop-blur-md border-b border-primary/10 px-4 md:px-10 lg:px-20 py-4">
        <div class="max-w-7xl mx-auto flex items-center justify-between">
            ...
            <nav class="hidden md:flex items-center gap-8">
                @if(auth()->check() && auth()->user()->role === \App\Models\User::ROLE_ADMIN)
                    <a href="{{ route('admin.dashboard') }}" class="hover:text-primary transition-colors text-sm font-medium text-slate-600">Dashboard_admin</a>
                @else
                    <a href="{{ url('/') }}" class="hover:text-primary transition-colors text-sm font-medium text-slate-600">Home</a>
                @endif
                <a href="{{ route('nurseries.index') }}" class="text-primary font-bold transition-colors text-sm font-medium">Nurseries</a>
                <a href="{{ route('explore') }}" class="hover:text-primary transition-colors text-sm font-medium text-slate-600">Explore</a>
                @if(auth()->check() && auth()->user()->role === \App\Models\User::ROLE_ADMIN)
                    <a href="{{ route('admin.requests') }}" class="hover:text-primary transition-colors text-sm font-medium text-slate-600">Requests</a>
                @else
                    <a href="{{ route('contact') }}" class="hover:text-primary transition-colors text-sm font-medium text-slate-600">Contact us</a>
                @endif
                @auth
